...
Here's old `rsyslog.sink.conf` that can work as a local RELP sink:
...
$WorkDirectory /tmp
$MainMsgQueueFileName relp_sink
$MainMsgQueueTimeoutShutdown 20000
$MainMsgQueueWorkerTimeoutThreadShutdown 10000
$MainMsgQueueMaxFileSize 50m
$MainMsgQueueSaveOnShutdown on

template(name="multilog" type="list") {
...
module(load="builtin:omfile" template="multilog" FileCreateMode="0640")

module(load="imrelp")
input(type="imrelp" name="relp" port="11122")

if 1 then action(type="omfile" createdirs="off" file="/tmp/relp.sink.log")
```

Run rsyslog with it via: `rsyslogd -n -f rsyslog.sink.conf -i /tmp/relp.sink.pid`\
(use `pkill -F /tmp/relp.sink.pid && rm /tmp/relp.sink.pid` to stop it later)
...
```
% syslog-ng -F -f syslog-ng.relp-client.conf \
 --persist-file /tmp/relp.client.queue \
 --pidfile /tmp/relp.client.pid --control /tmp/relp.client.ctl \
 < <(while :; do ((n++)); echo "Log line $n from $(date -Is)"; sleep 1; done)
```

(stop it with Ctrl-C or `pkill -F /tmp/relp.client.pid` later)

Then as syslog-ng RELP destination connects, it should be clear from syslog-ng
console output (enabled in [syslog-ng.relp-client.conf] in the repo here), and running
`tail -F /tmp/relp.sink.log` should immediately show lines generated by while-loop
in command above, as written there by rsyslog relp-receiver.
...
- [Reliable Event Logging Protocol spec] - as HTML file in [librelp github repo],
